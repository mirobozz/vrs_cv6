...
uint32_t LL_GetTick(void) { return uwTick; }

static void putstr(const char *s){ USART_Transmit((uint8_t*)s, (uint16_t)strlen(s)); }
static void putfmt(const char *fmt, ...){
    char b[96]; va_list ap; va_start(ap, fmt);
    int n = vsnprintf(b, sizeof b, fmt, ap); va_end(ap);
    if (n>0) USART_Transmit((uint8_t*)b, (uint16_t)((n<(int)sizeof b)?n:sizeof b));
}
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* WHO test */
     uint8_t who_h = i2c_read_reg(HTS221_ADDR_7B, HTS221_WHO_AM_I_REG);
     uint8_t who_p = i2c_read_reg(LPS22HB_ADDR_7B, LPS22HB_WHO_AM_I_REG);
     putfmt("WHO:%02X,%02X\r\n", who_h, who_p);

     /* init */
     uint8_t e1 = hts221_init();
     uint8_t e2 = lps22hb_init();
     LL_mDelay(20);
